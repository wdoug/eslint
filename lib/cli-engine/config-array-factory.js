...
function createConfigArray(elements, pluginBasePath) {
    const config = elements ? new ConfigArray(...elements) : new ConfigArray();

    config.pluginBasePath = pluginBasePath;
    return config;
}
...
class ConfigArrayFactory {
     ...
     */
    create(configData, { filePath, name, pluginBasePath } = {}) {
        const { resolvePluginsRelativeTo } = internalSlotsMap.get(this);
        ...
        return createConfigArray(
            configData
                ...
                : [],
            pluginBasePath || resolvePluginsRelativeTo
        );
    }
     ...
     * @returns {ConfigArray} Loaded config.
     */
    loadFile(filePath, { name, pluginBasePath } = {}) {
        const { cwd, resolvePluginsRelativeTo } = internalSlotsMap.get(this);
     ...
     */
    loadInDirectory(directoryPath, { name, pluginBasePath } = {}) {
        const { cwd, resolvePluginsRelativeTo } = internalSlotsMap.get(this);
    ...
    *_normalizeObjectConfigDataBody(
     ...
     * @private
     */
    _loadPlugin(name, importerPath, importerName, pluginBasePath) {
